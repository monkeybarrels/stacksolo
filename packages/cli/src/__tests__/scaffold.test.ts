...
describe('generateServiceScaffolds', () => {
  ...
  it('should scaffold container service with Dockerfile', () => {
    ...
    expect(filePaths).toContain('services/api/Dockerfile');
    expect(filePaths).toContain('services/api/package.json');
    expect(filePaths).toContain('services/api/tsconfig.json');
    expect(filePaths).toContain('services/api/src/index.ts');
    expect(filePaths).toContain('services/api/.gitignore');
  });
  ...
  it('should scaffold function service with handler', () => {
    ...
    const filePaths = result.files.map((f) => f.path);
    expect(filePaths).toContain('services/processor/package.json');
    expect(filePaths).toContain('services/processor/src/index.ts');
    // Functions don't have Dockerfile
    expect(filePaths).not.toContain('services/processor/Dockerfile');
  });
...
describe('generateScaffold', () => {
  it('should generate all files by default', () => {
    ...
    // Service files
    expect(filePaths.some((p) => p.startsWith('services/api/'))).toBe(true);

    // Summary
    expect(result.summary.envVars).toBeGreaterThan(0);
    expect(result.summary.dockerServices).toBe(2); // db + redis
  ...
  it('should only generate env files with --env-only', () => {
    ...
    expect(filePaths.some((p) => p.startsWith('services/'))).toBe(false);
  });

  it('should only generate docker-compose with --docker-only', () => {
    const config: StackSoloConfig = {
      project: {
        name: 'test-app',
        region: 'us-central1',
    ...
    const result = generateScaffold(config, { dockerOnly: true, targetDir: '.', force: false });
  ...
  it('should only generate services with --services-only', () => {
    ...
    expect(filePaths.some((p) => p.startsWith('services/'))).toBe(true);
    expect(filePaths).not.toContain('.env.local');
    expect(filePaths).not.toContain('docker-compose.yml');
  });
});
